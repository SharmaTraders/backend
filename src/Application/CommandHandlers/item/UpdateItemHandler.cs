...
public class UpdateItemHandler : IRequestHandler<UpdateItemCommand.Request> {
    ...
    public UpdateItemHandler(IItemRepository itemRepository, IUnitOfWork unitOfWork, IUniqueItemNameChecker uniqueItemNameChecker) {
    ...
    public async Task Handle(UpdateItemCommand.Request request, CancellationToken cancellationToken) {
       ...
       bool tryParse = Guid.TryParse(request.Id, out Guid id);
       if (!tryParse) {
